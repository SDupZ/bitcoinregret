...
export const updateExchangeRate = val => ({
  type: UPDATE_EXCHANGE_RATE,
  payload: val
});
export const updateCurrentExchangeRate = val => ({
  type: UPDATE_CURRENT_EXCHANGE_RATE,
  payload: val
});
export const updateTimeValue = val => ({
...
export const timeValueUpdated = val => (dispatch, getState) => {
  ...
  if (val < dayDifference) {
    ...
    if (Number(val) === 0) {
      ...
      fetchPrice(moment().add(-val, getState().value.timeUnit)).then(res => {
...
export const timeUnitUpdated = val => (dispatch, getState) => {
  ...
  dispatch(updateLoading(true));
  fetchPrice(moment().add(-getState().value.timeValue, val)).then(res => {
    dispatch(updateExchangeRate(res.bpi[Object.keys(res.bpi)[0]]));
  });
};
export const datetimeUpdated = val => dispatch => {
...
export default (state = initState, action) => {
  switch (action.type) {
    ...
    case UPDATE_EXCHANGE_RATE: {
      ...
      const percentage =
        ((newAmount - state.initialInvestment) / state.initialInvestment) * 100;
      return {
        ...state,
    ...
    case UPDATE_CURRENT_EXCHANGE_RATE: {
      const newAmount =
        state.initialInvestment * (1 / state.exchangeRate) * action.payload;
      const percentage =
        ((newAmount - state.initialInvestment) / state.initialInvestment) * 100;
      return {
        ...state,
    ...
    case UPDATE_DATETIME: {
      const timeVal = moment()
        .add(-state.timevalue, state.timeUnit)
        .diff(action.payload, state.timeUnit);
